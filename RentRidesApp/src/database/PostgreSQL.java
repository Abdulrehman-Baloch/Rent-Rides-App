...
public class PostgreSQL implements PersistenceHandler {
			...
			try (ResultSet resultSet = preparedStatement.executeQuery()) {
				while (resultSet.next()) {
					reservations.add(new Reservation(resultSet.getInt("reservationid"), resultSet.getInt("renterid"),
							resultSet.getInt("vehicleid"), resultSet.getInt("companyid"), resultSet.getInt("driverid"),
	...
	public static List<Driver> getAvailableCompanyDrivers(int companyID) {
	    List<Driver> drivers = new ArrayList<>();

	    // Logging the companyID to verify its value before query execution
	    System.out.println("Fetching available drivers for Company ID: " + companyID);

	    String query = "SELECT * FROM Driver WHERE companyID = ? AND status = 'Accepted' AND availabilityStatus = true";
	    ...
	    try (Connection connection = DriverManager.getConnection(url, username, password);
	        ...
	        try (ResultSet resultSet = preparedStatement.executeQuery()) {
	            while (resultSet.next()) {
	                drivers.add(new Driver(
	                    ...
	                    resultSet.getString("licenseNum"),
	                    resultSet.getBoolean("availabilityStatus"),
	        ...
	        System.err.println("Error loading available drivers for company ID " + companyID + ": " + e.getMessage());
	        e.printStackTrace();
	    }
	...
	public void updateUser(int userID, String selectedAttribute, String updatedValue, String role) {
		...
		case "Driver" -> "driverName, password, email, contactNumber, address,licenseNum";
