...
public class BrowseVehiclesController {
	...
	private void displayVehicles(List<Vehicle> vehiclesToDisplay) {
		...
		if (vehiclesToDisplay.isEmpty()) {
	        showAlert("No Results", "No vehicles available as of now.");
			vehicleListContainer.getChildren();
			return;
		}
		...
		for (Vehicle vehicle : vehiclesToDisplay) {
			try {
				...
				Text idLabel = new Text("Vehicle ID: " + String.valueOf(vehicle.getId()));
				idLabel.setStyle("-fx-font-weight: bold; -fx-font-size: 14px;");
				Text descriptionLabel = new Text(vehicle.getFeatures());
				descriptionLabel.setStyle("-fx-font-size: 14px;");
				Text priceLabel = new Text("Price: $" + vehicle.getPrice());
				priceLabel.setStyle("-fx-font-weight: bold; -fx-font-size: 14px;");
				Text locationLabel = new Text("Location: " + vehicle.getLocation());
	...
	private void searchVehicles() {
		...
		if (filteredVehicles.isEmpty()) {
	        showAlert("No Results", "No vehicles found matching your search criteria.");
			vehicleListContainer.getChildren().clear();
			vehicleListContainer.getChildren();
		} else {
