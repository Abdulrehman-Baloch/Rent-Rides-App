...
public class ProcessPaymentController {
		...
		Platform.runLater(() -> {
			...
			stage.setOnCloseRequest(event -> {
				if (!isPaymentCompleted()) {
					showAlertAndClose("Payment Incomplete", "Payment was not completed. The vehicle is now available.");
					try {
	...
	private void onCalculateButtonClick() {
		try {
			...
			showAlert("Error", "Error calculating amount: " + e.getMessage());
		}
	}
	...
	private void addTooltips() {
		...
		easypaisaPhoneNumberField.setTooltip(new Tooltip("Enter a valid Easypaisa phone number (starts with '03' and 11 digits long)"));
	}
	...
	private void handlePayment() throws SQLException {
		...
		try {
			...
			showAlert("Error", "Error processing payment: " + e.getMessage());
			paymentSuccess = false;
		}

		if (paymentSuccess) {
			showAlertAndClose("Payment Successful", "Payment Successful using " + selectedPaymentMethod);
			updateReservationPaymentStatus(true);
			clearPaymentFields();
			loadRentrMenuScreen();
		} else {
			showAlert("Error", "Invalid " + selectedPaymentMethod + " details");
		}
	}
	...
	public void loadRentrMenuScreen() {
		try {
			...
			e.printStackTrace();
			showAlertAndClose("Error", "Error loading the renter menu screen: " + e.getMessage());
		}
	}
}
